...
function init_new_task_comment_2(manual) {
    ...
    taskCommentAttachmentDropzone = new Dropzone("#task-comment-form-2", appCreateDropzoneOptions({
        uploadMultiple: true,
        ...
        paramName: 'file',
        sending: function (file, xhr, formData) {
            formData.append("taskid", $('#addTaskCommentBtn-2').attr('data-comment-task-id-2'));
            if (tinyMCE.activeEditor) {
                formData.append("content", tinyMCE.activeEditor.getContent());
            } else {
                formData.append("content", $('#task_comment-2').val());
            }
            formData.append("content", $('#moment-2').val());
        },
        success: function (files, response) {
            response = JSON.parse(response);
            if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
