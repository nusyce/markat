...
$('.removedd').click(function (e) {
    ...
    var itemSelectorOption = $(this).parents('.input-group');
    $item = itemSelectorOption.selectpicker('val');
    $itemv = itemSelectorOption.find('select').val();
    if (confirm_delete() && $item) {
        $.get(admin_url + 'misc/rm_project/' + $itemv).done(function (response) {
            itemSelectorOption.find('option:selected').remove();
            itemSelectorOption.selectpicker('refresh');
        });
    }
...
function misc_add_inline_select_submit(type) {
    ...
    if (val !== '') {
        ...
        data.name = val;
        data.inline = true;
        $.post(admin_url + 'misc/project', data).done(function (response) {
            response = JSON.parse(response);
            if (response.success === true || response.success == 'true') {
                var select = $('body').find('select#' + type);
                if (response.data.state == 'old') {
                    select.selectpicker('val', response.data.id);
                } else {
                    select.append('<option value="' + response.data.id + '">' + val + '</option>');
                    select.selectpicker('val', response.data.id);
...
$(function () {
    ...
    function displayTime() {
        var time2 = moment().tz(app.options.timezone).format('dddd, DD.MM.YYYY | HH:mm');
        $('#clock2').html(time2 + ' Uhr');
    ...
    // On click on task comment textarea make it tinymce, by default is plain textarea
    $("body").on('click focus', '#task_comment-2', function (e) {
        init_new_task_comment_2();
    });
    ...
    // Check if calendar exists in the DOM and init.
    if (calendar_selector_d.length > 0) {
        validate_calendar_form();
        var calendar_settings = {
        ...
        // Init calendar
        calendar_selector_d.fullCalendar(calendar_settings);
    }
    ...
    if ($('.field-cloneb').length > 0) {
        //  f();
        /*        $('#wohnungen, #mieter').on("keyup keypress click  blur change", ".enddate, .startdate", function (e) {
                    f();
                });*/
    ...
    $('#add-div a').click(function (e) {
        ...
        $cloned.find('input').val(0);
        $cloned.find('.remove-aq').removeClass('disabled');
        $cloned.insertBefore('#add-div');
        init_selectpicker();
        cunbt();
    });
    $("#wohnungen").on("click", ".simple .remove-aq", function (e) {
        e.preventDefault();
        var parent = $(this).parents('.field-clone');
        var countParent = $('.field-clone').length;
        if (countParent > 1) {
    ...
    function cunbt() {
        var biland = 0;
        $('.count_cone').each(function (i, obj) {
            $(this).find('.count-k').html(i + 1);
            $res = parseInt($(this).find('.a_qty').val());
            if ($res) {
                biland += $res;
                $('#inventarCOunt').html(biland)
            }
        });
    }

    $('#wohnungen, #mieter').on('change', '.a_qty', function (event) {
        cunbt();
        var parentsd = $(this).parents('.count_cone');
        $austattung = parentsd.find('.austattungSelect');
        $austattung.selectpicker('refresh')
        $austattung = $austattung.find('select').val();
        console.log($austattung)
        if ($austattung)
            changedd($austattung, parentsd)
    });
    $('#wohnungen, #mieter').on('change', '.austattungSelect', function (event) {
        var _this = $(this);
        var parent = $(this).parents('.count_cone');
        changedd(this.value, parent)
    });

    function changedd($value, $parent) {
        $.ajax({
            type: 'get',
            url: admin_url + 'wohnungen/get_inventar_ajax/' + $value,
            success: function (response) {
                response = JSON.parse(response);
                if (response) {
                    $qty = parseInt($parent.find('.a_qty').val());
                    if (response.qubik > 0) {
                        $parent.find('.sqr').val(response.qubik * $qty);
                    } else {
                        $parent.find('.sqr').val($qty);
                    }
                }
            }
        });
    ...
    $('#wohnungen, #mieter').on("click", ".reasean .remove-aq", function (e) {
        e.preventDefault();
        var parent = $(this).parents('.field-clone');
        var countParent = $('.field-clone .firstroun').not(".hide").length;
        $inventar_id = parent.data('id');
        if (countParent > 1) {
            $('#removeInventar').modal('show');
            $('#inventar_id').val($inventar_id);
        }
    })

    $('#gestreason').click(function (e) {
        e.preventDefault();
        $inventar = $('#inventar_id').val();
        $('#removeInventar').modal('hide');
        $reason = $("input[name='whyaction']:checked").val();
        var inBlock = $('div[data-id="' + $inventar + '"]');
        inBlock.find('.deleted').val(1);
        inBlock.find('#deleted-reason').removeClass('hide');
        $oldR = inBlock.find('.reasonsd').val();
        inBlock.find('.reasonsd').val($reason);
        inBlock.find('.firstroun').addClass('hide');

        $('#removeInventar').on('hidden.bs.modal', function () {
            $('#action-inventar').trigger('reset');
        })
    })
    ...
    if (table_leads.length) {
        ...
        var belegunTableServer = [0, table_leads.find('#th-assigned').index()];

        if (tableLeadsConsentHeading.length > 0) {
            leadsTableNotSortable.push(tableLeadsConsentHeading.index());
            belegunTableServer.push(tableLeadsConsentHeading.index());
        }

        _table_api = initDataTable(table_leads, admin_url + 'leads/table', belegunTableServer, leadsTableNotSortable, LeadsServerParams, [table_leads.find('th.date-created').index(), 'desc']);
    ...
    /*
        $('#wohnungen .mieter_p').on('change', function () {
            var _this = $(this);
            $.ajax({
                type: 'get',
                url: admin_url + 'mieter/get_ajax/' + this.value,
                success: function (mieter) {
                    mieter = JSON.parse(mieter);

                    if (mieter.active) {
                        _this.parents('.field-cloneb').find('.startdate').val(mieter.belegt_v);
                        _this.parents('.field-cloneb').find('.enddate').val(mieter.belegt_b);
                        _this.parents('.field-cloneb').find('.resttage').val(mieter.resttage);
                        _this.parents('.field-cloneb').find('.ausstehend').val(mieter.ausstehend);
                    }
                }
            });
        });*/
    ...
    if (table_mieter.length) {
        ...
        var _table_api = initDataTable(table_mieter, admin_url + 'mieter/table', [0], [0], LeadsServerParams, [1, 'desc'], filterArray);
    ...
    //Init the Project-Mieters table
    var table_project_mieter = $('#table-project-mieter');
    if (table_project_mieter.length) {
        let url = table_project_mieter.data('url');
        let filterArray = [];
        var LeadsServerParams = {};
        initDataTable(table_project_mieter, url, [0], [0], LeadsServerParams, [1, 'desc'], filterArray);
    }

    //Init the Project-Mieters table
    var table_project_aq = $('#table-project-aq');
    if (table_project_aq.length) {
        let url = table_project_aq.data('url');
        initDataTable(table_project_aq, url, [0], [0], [], [1, 'desc'], []);
    }
    //Init the Project-Mieters table
    var table_project_staff = $('#table-project-staff');
    if (table_project_staff.length) {
        let url = table_project_staff.data('url');
        initDataTable(table_project_staff, url, [0], [0], [], [1, 'desc'], []);
    }

    //Init the Project-Mieters table
    var table_project_rb = $('#table-project-rb');
    if (table_project_rb.length) {
        let url = table_project_rb.data('url');
        initDataTable(table_project_rb, url, [0], [0], [], [1, 'desc'], []);
    }

    //Init the Project-Mieters table
    var table_project_bp = $('#table-belegungsplan-ps');
    if (table_project_bp.length) {
        let url = table_project_bp.data('url');
        initDataTable(table_project_bp, url, [0], [0], [], [1, 'desc'], []);
    }

    //Init the Project-Mieters table
    var table_project_ps = $('#table-project-ps');
    if (table_project_ps.length) {
        let url = table_project_ps.data('url');
        initDataTable(table_project_ps, url, [0], [0], [], [0, 'desc'], []);
    }

    // Init the table
    var table_wohnungen = $('.table-wohnungen');
    if (table_wohnungen.length) {
        // Add additional server params $_POST
        var LeadsServerParams = {
            "belegt": "[name='belegt']",
            "strabe": "[name='strabe']",
        ...
        var _table_api = initDataTable(table_wohnungen, admin_url + 'wohnungen/table', [0], [0], LeadsServerParams, [1, 'desc'], filterArray);
    ...
    // Init the table
    var table_visualisierung = $('.table-visualisierung');
    if (table_visualisierung.length) {
        // Add additional server params $_POST
        var LeadsServerParams = {
            "belegt": "[name='belegt']",
            "strabe": "[name='strabe']",
        ...
        var _table_api = initDataTable(table_visualisierung, admin_url + 'visualisierung/table', [0], [0], LeadsServerParams, [1, 'desc'], filterArray);

        $.each(LeadsServerParams, function (i, obj) {
            $('#' + i).on('change', function () {
                table_visualisierung.DataTable().ajax.reload()
                    .columns.adjust()
                    .responsive.recalc();
            });
        });
    ...
    $("body").on('change', '.multiple_action input[type=checkbox]', function () {
        blukenabled();
    })

    function blukenabled() {
        var count = $(".multiple_action  input[type=checkbox]:checked").length;

        if (count > 1) {
            $('#wrapper').addClass('my-container');
        } else {
            $('#wrapper').removeClass('my-container');
        }
    }
...
$('#sqdsqd').click(function (e) {
    ...
    if (confirm_delete()) {
        bulk_remove(model, post_arr);
    }
    return false;
})

function bulk_remove(modele, post_arr) {
    $.post(admin_url + modele + '/bulk_delete', {data: post_arr}).done(function (response) {
...
function initDataTable(selector, url, notsearchable, notsortable, fnserverparams, defaultorder, filterArray = []) {
...
function _gen_misc_add_inline_on_select_field(type) {
    var html = '';
    html = "<div id=\"new_misc_" + type + "_inline\" class=\"form-group\"><label for=\"new_" + type + "_name\">" + $('label[for=\"' + type + '\"]').html().trim() + "</label><div class=\"input-group\"><input type=\"text\" id=\"new_" + type + "_name\" name=\"new_" + type + "_name\" class=\"form-control\"><div class=\"input-group-addon\"><a href=\"#\" onclick=\"misc_add_inline_select_submit('" + type + "'); return false;\" class=\"lead-add-inline-submit-" + type + "\"><i class=\"fa fa-check\"></i></a></div></div></div>";
    $('.form-group-select-input-' + type).after(html);
    $('body').find('#new_' + type + '_name').focus();
...
function new_project_status_inline() {
    _gen_misc_add_inline_on_select_field('project');
}
...
if ($('#belegungsplan').length > 0) {
    ...
    $('#newoccupation').on('change', '.startdate, .enddate, .efilter, .sfilter, .mfilter ', function (event) {
        ...
        if (parseInt($('.belegungsplan_id').val()) > 0)
            return;
        requestGet('belegungsplan/load_free_aq/' + a + '/' + b + '/' + c + '/' + d + '/' + e).done(function (response) {
            response = JSON.parse(response);
...
$('.need-edition').on('click', '.edit-menu', function (e) {
    ...
    $('#modal-edit-menu #menu_slug').val($(this).parents('.need-edition').data('target').trim());
...
var message_no_select = 'Select the items to delete';
...
// Removes task single attachment
function remove_staff_attachment(link, id) {
    if (confirm_delete()) {
        requestGetJSON('staff/delete_attach/' + id).done(function (response) {
            if (response) {
                $('[data-staff-attachment-id="' + id + '"]').remove();
            }
        });
    }
}
...
function similar_task(task_id) {
    requestGet('tasks/copy_task/' + task_id).done(function (response) {
        $('#_task').html(response);
        $('#task-modal').modal('hide');
        $("body").find('#_task_modal').modal({show: true, backdrop: 'static'});
        reload_tasks_tables();
...
// General function to append task html returned from request
function _task_append_html_2(html) {
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
    ...
    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
        ...
        table_row += '</td>';
        table_row += '<td class="text-center">' + item_key + '</td>';
...
function init_new_task_comment_2(manual) {

    if (tinymce.editors.task_comment) {
        tinymce.remove('#task_comment-2');
    }

    if (typeof (taskCommentAttachmentDropzone_2) != 'undefined') {
        taskCommentAttachmentDropzone_2.destroy();
    }
    $('#dropzoneTaskComment-2').removeClass('hide');
    $('#addTaskCommentBtn-2').removeClass('hide');
    ...
    // Not working fine on iOs
    if (!iOS) {
        init_editor('#task_comment-2', editorConfig);
    }
}
