...
class Clients_model extends App_Model
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['client_vault_entries_model', 'mieter_model', 'client_groups_model', 'statement_model']);
    }
    ...
    {
        $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'clients')) . ',' . get_sql_select_client_company(), ['email']);
    ...
    public function get_mieter_number($customer_id)
    {
        $contacts = $this->get_contacts($customer_id);
        $count = 0;
        foreach ($contacts as $contact) {
            $count += count($this->mieter_model->get_betreuer_by($contact['id']));
        }
        return $count;
    }
     ...
     * Add new occupations
     */
    public function add_ansprechpartner($data)
    {
        if ($data) {
            ...
            if (isset($data['ansprechpartner_id']))
                unset($data['ansprechpartner_id']);

            $this->db->insert(db_prefix() . 'ansprechpartner', $data);
            $insert_id = $this->db->insert_id();

            log_activity('New ansprechpartner Added [' . $data['id'] . ']');
    ...
    public function update_ansprechpartner($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'ansprechpartner', $data);
     ...
     * @return boolean
     */
    public function assign_admins_old($data, $id)
    {
        $affectedRows = 0;
        if (count($data) == 0) {
            $this->db->where('userid', $id);
            $this->db->update(db_prefix() . 'contacts', ['userid' => 0]);
        } else {
            $current_admins = $this->get_admins($id);
            $current_admins_ids = [];
            foreach ($current_admins as $c_admin) {
                array_push($current_admins_ids, $c_admin['id']);
            }
            foreach ($current_admins_ids as $c_admin_id) {
                if (!in_array($c_admin_id, $data['customer_admins'])) {
                    $this->db->where('id', $c_admin_id);
                    $this->db->update(db_prefix() . 'contacts', ['userid' => 0]);
                }
            }
            foreach ($data['customer_admins'] as $n_admin_id) {
                $this->db->where('id', $n_admin_id);
                $this->db->update(db_prefix() . 'contacts', ['userid' => $id]);
            }
        }
        if ($affectedRows > 0) {
            return true;
