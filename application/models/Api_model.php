...
class Api_model extends App_Model
{
    ...
    public function login($username, $password)
    {
        ...
        if($user)
        {
            ...
            if (app_hasher()->CheckPassword($password, $user_pass))
            {
                return $user;
            }else{
                return FALSE;
    ...
    public function get_staks($user_id)
{

    $this->db->select('taskid');
    $this->db->where('staffid', $user_id);
    $mes_task= $this->db->get(db_prefix() . 'task_assigned')->result_array();
    $task=[];
    foreach ($mes_task as $item)
    {
        array_push($task,$item['taskid']);
    }
    if ($task==[])
    {
        return [];
    ...
    $this->db->where_in(db_prefix() . 'tasks.id', $task);

    $this->db->select(db_prefix() . 'tasks.id as id,'.
        ...
        db_prefix() . 'projects.name as project,
           priority');
    $this->db->join(db_prefix() . 'projects', db_prefix() . 'projects.id ='. db_prefix() . 'tasks.project', 'left');
    $this->db->join(db_prefix() . 'mieters', db_prefix() . 'mieters.id ='. db_prefix() . 'tasks.mieters', 'left');
...
public function get_notifications_api($id)
{
    $this->db->where(db_prefix() . 'notifications.touserid', $id);
    $this->db->where(db_prefix() . 'notifications.isread',0);

    //$this->db->like(db_prefix() . 'notifications.date', date("Y-m-d"));
    $result= $this->db->order_by('id',"desc")->limit(1)->get(db_prefix() . 'notifications')->row();
    $result->value=_l(  $result->description);
    $result->additional_data=unserialize($result->additional_data);
    return $result;
}
    public function get_staks_detail($id,$user="")
    {


        $this->db->where(db_prefix() . 'tasks.id', $id);

        $this->db->select(db_prefix() . 'tasks.description as description,'.db_prefix() . 'tasks.id as id,'.
            db_prefix() . 'tasks.name as task_name,'.
            db_prefix() . 'tasks.status as status,'.
            'startdate,
           duedate,'.
           ...
           ,datefinished,'.
            db_prefix() . 'projects.name as project,
           priority,'.
            db_prefix() . 'mieters.id as idm,'.get_sql_select_task_asignees_full_names() . ' as assignees,(SELECT count(' . db_prefix() . 'task_checklist_items.taskid) FROM ' . db_prefix() . 'task_checklist_items WHERE ' . db_prefix() . 'task_checklist_items.taskid=' . db_prefix() . 'tasks.id) as checkpoint');
        $this->db->join(db_prefix() . 'projects', db_prefix() . 'projects.id ='. db_prefix() . 'tasks.project', 'left');
        $this->db->join(db_prefix() . 'mieters', db_prefix() . 'mieters.id ='. db_prefix() . 'tasks.mieters', 'left');
        $this->db->join(db_prefix() . 'task_assigned', db_prefix() . 'task_assigned.taskid ='. db_prefix() . 'tasks.id', 'left');
        $result=$this->db->get(db_prefix() . 'tasks')->row();

        $this->db->select('concat('.db_prefix() . 'staff.firstname," ",'.db_prefix() . 'staff.lastname) as assingner');
        $this->db->where(db_prefix() . 'task_assigned.taskid', $id);
        $this->db->where(db_prefix() . 'task_assigned.staffid', $user);
        $this->db->join(db_prefix() . 'task_assigned', db_prefix() . 'task_assigned.assigned_from='. db_prefix() . 'staff.staffid', 'left');
        $assigner=$this->db->get(db_prefix() . 'staff')->row();
        $result->assigner=$assigner->assingner;
        return $result;
