...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['title'] = get_menu_option(c_menu(), 'Kunder');
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            $data['ansprechpartners'] = $this->clients_model->get_ansprechpartners($id);
            if (!$client) {
                show_404();
            }
    ...
    public function ansprechpartner($customer_id)
    {
        if ($this->input->post()) {
            if ($_POST['ansprechpartner_id'] == '0') {
                $id = $this->clients_model->add_ansprechpartner($this->input->post());
                if ($id) {
                    set_alert('success', _l('added_successfully', 'Ansprechpartner'));
                    redirect(admin_url('clients/client/' . $customer_id) . '?tab=ansprechpartner');
                }
            } else {

                $success = $this->clients_model->update_ansprechpartner($this->input->post(), $_POST['ansprechpartner_id']);
                if ($success) {
                    set_alert('success', _l('updated_successfully', 'Ansprechpartner'));
                }
                redirect(admin_url('clients/client/' . $customer_id) . '?tab=ansprechpartner');
            }
        }
    ...
    {
        $response = $this->clients_model->get_ansprechpartner($id);
        echo json_encode($response);
        exit();
    }

    public function delete_ansprechpartner($customer_id, $id)
    {
        $this->clients_model->delete_ansprechpartner($id);
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        if ($customer_id == 0)
            redirect(admin_url('clients/all_contacts'));
        else
