...
class Tasks extends AdminController
{
    ...
    public function list_tasks($id = '')
    {
        ...
        $data['staff'] = $this->misc_model->get_tasks_distinct_assignees();
    ...
    public function duplicate($id)
    {
        ...
        $newTask['datefinished'] = $task->datefinished;
        $newTask['comments'] = $task->comments;
        $task = $this->tasks_model->duplicate($newTask);
        var_dump($task);
    }
    ...
    public function print_pdf()
    {
        ...
        $task = $this->tasks_model->get($_POST['task_id']);
        $task->signature = $_POST['imageData'];
        try {
    ...
    {
        $tasks_my_where = 'id IN(SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid=' . get_staff_user_id() . ')';
    ...
    public function bulk_delete()
    {
        ...
        if (isset($_POST['data'])) {
            ...
            if (count($total_deleted) > 0) {
                ...
                set_alert('warning', _l('problem_deleting', 'task'));
            }
            echo admin_url('tasks');
        } else {
            set_alert('warning', _l('problem_deleting', 'task'));
            echo false;
        }
